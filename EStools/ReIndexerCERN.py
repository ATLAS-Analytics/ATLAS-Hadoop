...
ce = Elasticsearch([{'host':'es-atlas.cern.ch', 'port':9202}],http_auth=('es-atlas', 'yourpassword'), sniff_on_start=True, sniff_on_connection_fail=True, sniffer_timeout=60, timeout=600)
sources=[]
sources.append( 'faxcost-2016.01.01-10')

for s in sources:
    print(s)
    res = helpers.reindex(es, s, s,target_client=ce)
