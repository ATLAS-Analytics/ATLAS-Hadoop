...
PAN = LOAD '/atlas/analytics/panda/JOBSARCHIVED/jobs.2015-02' USING AvroStorage();
DESCRIBE PAN;
...
P = foreach PA generate TRANSFERTYPE,(long)MODIFICATIONTIME, JOBSTATUS,  STARTTIME, ENDTIME, COMPUTINGSITE, DESTINATIONSE, CLOUD, REPLACE(SOURCESITE,'ANALY_','') as SOURCE, DESTINATIONSITE, (int)PILOTERRORCODE, (int) NINPUTFILES, ENDTIME - STARTTIME as WALLTIME, STARTTIME - CREATIONTIME as WAITTIME;


jobEffGroup = GROUP P BY (TRANSFERTYPE,COMPUTINGSITE, JOBSTATUS);
jeg = FOREACH jobEffGroup GENERATE group, COUNT(P.MODIFICATIONTIME), AVG(P.WALLTIME)/1000, AVG(P.WAITTIME)/100;
dump jeg;
